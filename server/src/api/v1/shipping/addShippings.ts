...
async function insertShipping(shipping: IShipping) {
    try {
        ...
        return result.acknowledged ? result.insertedId : "Failed while inserting shippings";
    } catch (err) {
...
async function sendFailedEntries(failedEntries: TFailedEntry[], email: string) {
    const sheet = await generateExcelSheet(CollectionNames.Shippings, failedEntries);

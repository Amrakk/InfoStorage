...
import { CollectionNames, ProductCategories } from "../../../configs/default.js";
import {
    generateExcelFile,
    generateExcelSheet,
...
export const addProducts = employeeProcedure.input(inputSchema).mutation(async ({ input, ctx }) => {
...
async function insertProduct(product: IProduct) {
    try {
        ...
        return result.acknowledged ? result.insertedId : "Failed while inserting products";
    } catch (err) {
...
async function sendFailedEntries(failedEntries: TFailedEntry[], email: string) {
    const sheet = await generateExcelSheet(CollectionNames.Products, failedEntries);
